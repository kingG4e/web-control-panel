...
const CircleStackIcon = ({ className, style }) => (
  <svg className={className} style={style} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const CpuChipIcon = ({ className, style }) => (
  <svg className={className} style={style} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Clear services state first to force re-render
      setServices([]);
      ...
      // Fetch system stats
      const systemResponse = await fetch(`/api/dashboard/system-stats?t=${timestamp}`, {
        headers: {
          'Cache-Control': 'no-cache',
          'Pragma': 'no-cache'
        }
      ...
      if (systemResponse.ok) {
        ...
        console.log('System stats response:', systemData);
        if (systemData.success && systemData.data) {
          setSystemStatus(systemData.data);
        }
      }

      // Fetch services with cache-busting
      console.log('Fetching services data...');
      const servicesResponse = await fetch(`/api/dashboard/services?t=${timestamp}`, {
        headers: {
          'Cache-Control': 'no-cache',
          'Pragma': 'no-cache'
        }
        ...
        if (servicesResponse.ok) {
          ...
          console.log('Services data array:', servicesData.data);
          if (servicesData.success && servicesData.data) {
            console.log('Setting services:', servicesData.data);
          ...
          console.error('Services response not OK:', servicesResponse.status);
  ...
  const ServiceStatus = ({ service }) => {
    // Add debugging
    console.log('ServiceStatus component received service:', service);
        ...
        {stats.isAdmin && (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '16px' }}>
              <ShieldCheckIcon style={{ width: '24px', height: '24px', color: '#f59e0b', marginRight: '12px' }} />
      ...
      {/* Stats Grid */}
        ...
        <StatCard
          title="Virtual Hosts"
            ...
            icon={<ServerIcon style={{ width: '24px', height: '24px', color: '#60a5fa' }} />}
            color="#60a5fa"
        />
        <StatCard
          title="Databases"
            ...
            icon={<CircleStackIcon style={{ width: '24px', height: '24px', color: '#34d399' }} />}
            color="#34d399"
        />
        <StatCard
        ...
        <StatCard
          title="SSL Certificates"
            ...
            icon={<ShieldCheckIcon style={{ width: '24px', height: '24px', color: '#f87171' }} />}
            color="#f87171"
        />
      </div>
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
              <CpuChipIcon style={{ width: '24px', height: '24px', color: '#60a5fa', marginRight: '12px' }} />
              <h3 style={{ fontSize: '18px', fontWeight: '600', color: 'var(--primary-text)', margin: 0 }}>
                System Resources
              </h3>
            </div>
